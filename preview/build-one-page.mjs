...
const locations = {
  shenzhen: {
    name: 'Shenzhen',
    art: 'assets/pixel-art/shenzhen-modern.webp',
    alt: 'Pastel pixel-art view of present-day Shenzhen with a bayfront skyline, metro, promenade, and evening light',
    kind: 'modern'
  },
  guangzhou: {
    name: 'Guangzhou',
    art: 'assets/pixel-art/guangzhou-modern.webp',
    alt: 'Pastel pixel-art view of present-day Guangzhou with Canton Tower, the Pearl River, ferries, and a modern skyline',
    kind: 'modern'
  },
  guilin: {
    name: 'Guilin',
    art: 'assets/pixel-art/guilin.webp',
    alt: 'Pastel pixel-art view of Guilin karst peaks, misty water, twin pagodas, and a bamboo raft',
    kind: 'landscape'
  },
  xingping: {
    name: 'Xingping',
    art: 'assets/pixel-art/xingping.webp',
    alt: 'Pastel pixel-art view of Xingping Ancient Town, the Li River, karst peaks, a bamboo raft, and a hanfu-clad traveler',
    kind: 'landscape'
  },
  chongqing: {
    name: 'Chongqing',
    art: 'assets/pixel-art/chongqing-modern.webp',
    alt: 'Pastel pixel-art view of present-day Chongqing with a monorail, high-rises, bridges, layered roads, and river traffic at blue hour',
    kind: 'modern'
  },
  chengdu: {
    name: 'Chengdu',
    art: 'assets/pixel-art/chengdu-modern.webp',
    alt: 'Pastel pixel-art view of present-day Chengdu with a bamboo-edged park, teahouse terrace, metro, cyclists, and a modern skyline',
    kind: 'modern'
  }
};
...
const locationArtwork = Object.entries(locations).map(([key, location], index) => {
  const first = index === 0;
  const caption = location.kind === 'modern' ? 'Modern city portrait' : 'Landscape interlude';
  return `
      <figure class="location-art${first ? ' active' : ''}" data-location-art="${key}-${location.kind}" data-location="${key}" data-era="${location.kind}"${first ? '' : ' aria-hidden="true"'}>
        <img src="${location.art}" alt="${location.alt}" ${first ? 'fetchpriority="high"' : 'loading="lazy"'} decoding="async"/>
        <figcaption><span>${String(index + 1).padStart(2, '0')} · ${location.name}</span><small>${caption}</small></figcaption>
      </figure>`;
}).join('');
  ...
  <main>
    <section class="journey-hero" id="overview">
      <div class="hero-grid wrap">
        ...
        <aside class="hero-note" aria-label="Journey direction"><span>South → West</span><strong>22°N<br/>to 30°N</strong><p>Modern cities · quiet landscapes.</p></aside>
      </div>
    </section>
    ...
    <section class="itinerary-intro" id="itinerary"><div class="wrap"><p class="pixel-label">Scroll the journey</p><div><h2>Day by day</h2><p>On larger screens, each modern city or landscape portrait stays beside its days. On mobile, swipe the visual route once, then read without an image covering the itinerary.</p></div></div></section>

    <div class="itinerary-stage">
      <div class="location-visuals">
        <p class="mobile-route-label"><strong>Six stops in pictures</strong><span>Swipe the route →</span></p>
...
const artworkCount = Object.keys(locations).length;
